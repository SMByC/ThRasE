...
It includes a registry system that tracks all modifications made during editing sessions. The plugin also features
comprehensive navigation tools that enable systematic and thorough layer inspection, a critical aspect of maintaining
data quality and accuracy in thematic mapping projects.
...
with reference data. However, the plugin is not limited to this and can be used for various other thematic mapping tasks.
...
1. **Thematic with paletted or singleband pseudocolor on the fly**:
   ...
   or symbology applied on the fly in QGIS.
...
2. **Thematic with embedded color table**:
   ...
   You can use any raster (with byte or integer data type) that has pixel-value/color associations stored in a color table
...
## Registry - Modification Tracking System

<img src="img/registry.webp" width="85%" style="margin: auto;display: block;">

ThRasE includes a Registry system that tracks all modifications made to your thematic raster during editing sessions.

- **Edit History**: Every pixel modification is automatically recorded with timestamps, allowing you to review the
entire editing process. When using lines, polygons, or freehand tools, ThRasE groups the edited pixels by these edit actions.
- **Visual Modifications**: You can see all the modified areas highlighted on the canvas, making it easy to see which regions
have been edited.
- **Timeline Navigation**: Use the registry slider to navigate through different stages of your editing process.
- **Selective Recording**: Choose whether to record changes for specific operations by enabling/disabling the registry.
- **Export Capabilities**: Save your registry to a vector file with all pixel modifications, including the original and
new pixel values, and timestamps of edits.

<img src="img/export_registry.webp" width="85%" style="margin: auto;display: block;">

> **Note:** While the registry is enabled (by default), ThRasE will save the registry in the configuration file (YAML) and
the registry will be restored from it. Take into account that the registry can grow over time for large edits. If that makes
ThRasE slow down, consider disabling the registry or using different editing sessions. You can restart the registry by
clearing it.

This Registry system ensures that your editing workflow is reliable, giving you confidence to make modifications while
maintaining the ability to inspect, review, or export changes.
...
- **Apply from Thematic File Classes**: This option allows you to selectively apply changes to specific classes from
  another thematic file. This is particularly useful for targeted updates based on another classification:
...
Google Earth Engine while you edit your thematic raster.

- The CCD plugin provides:
  - The ability to analyze temporal patterns at specific points of interest
