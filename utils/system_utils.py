...
def error_handler(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            msg_error = "An error has occurred in ThRasE plugin. " \
                        "See more in Qgis log messages panel."
            iface.messageBar().pushMessage("ThRasE", msg_error,
                                           level=Qgis.Critical, duration=10)
            # message in log
            msg_error = "\n################## ERROR IN ThRasE PLUGIN:\n"
            msg_error += traceback.format_exc()
            msg_error += "\nPlease report the error in:\n" \
                         "\thttps://bitbucket.org/smbyc/qgisplugin-thrase/issues"
            msg_error += "\n################## END REPORT"
